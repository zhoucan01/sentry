...
//vvvvv++;
    ...
    {
		//传输发生串口空闲中断后，首先需要清除SR和DR寄存器，
		//因为他们还有数据的话，下一次启动DMA传输，他们会被DMA直接搬走
		//所以在发生DMA空闲中断后，要清除这2个寄存器，直接读取就可以清除。
...
//		__HAL_DMA_DISABLE(huart1.hdmarx);
             ...
             iiiii=huart1.hdmarx->Instance->NDTR;
             temp_1=temp;
//             iiiii=huart1.hdmarx->Instance->NDTR;
		//RX_BUFFER_COUNT要设置的比实际要接收的多，CHASSIS_Rx_buff_count是实际接受的字节数
		if((DATA_COUNT_RX- temp) == DATA_RX_count)
		{
    
    RX_USART_data_Handle(USART_Rx_data_handle,&USART_Rx_data);
//    RX_USART_data_Handle(USART_Rx_data_handle,&USART_Rx_data);
