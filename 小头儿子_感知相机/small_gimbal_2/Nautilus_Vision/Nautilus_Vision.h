...
#define VISION_RX_COUNT 17
#define VISION_TX_COUNT 24
...
typedef struct
{
  ...
  uint8_t robot_color;
	uint8_t Vision_Mode;
	uint8_t shoot_speed;
  uint8_t top_senior;//最高优先级车体
  uint8_t if_outpost_lock;//前哨站是否停转 1是静止
	float roll;
  float pitch;
	float yaw;
...
/********************同济视觉通信************************/
#define Tj_Tx_count 43
#define Tj_Rx_count 29
...
typedef enum
{
  ...
  control_nfre,
  control_fire,
}Vision_gimbal_mode_t;

typedef struct
{
  uint8_t head[2];
  Vision_gimbal_mode_t Vision_gimbal_mode; //0 不控制  1控制但不开火  2控制且开火
  float yaw;
  float yaw_vel;
  float yaw_acc;
  float pitch;
...
typedef enum
{
  ...
  gimbal_small_buff,
  gimbal_big_buff,
}Vision_Tx_gimbal_mode_t;

typedef struct
{
  uint8_t head[2];
  Vision_Tx_gimbal_mode_t Vision_Tx_gimbal_mode;    //0：空闲  1: 自瞄  2:小符  3:大符
  float q[4];
  float yaw;
  float yaw_vel; 
  float pitch;
...
extern  TJ_Vision_Rx_t TJ_Vision_Rx;
extern TJ_Vision_Tx_t TJ_Vision_Tx;
//enum ARMOR_NUM
//{
//    ARMOR_NUM_BALANCE = 2,
//    ARMOR_NUM_OUTPOST = 3,
...
extern pid_struct_t pid_outpost_angle;
extern pid_struct_t pid_outpost_speed;
