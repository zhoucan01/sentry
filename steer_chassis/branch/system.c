...
chassis_t chassis=
{
	.last_ecd[FR]=7662,/*舵机初始化使底盘朝向正方向,即电机装上时的偏置角*/
	.last_ecd[BR]=5791,
	.last_ecd[BL]=5218,//2380
	.last_ecd[FL]=3132,
	.Vz_state=0,
	.speed_direct[0]=1,
	.speed_direct[1]=1,
...
first_order_filter_init(&filter_pm,0.02,&pm_kp);
	pid_init(&pid_chassis_follow,7500,140,0,0,3000);
	for(;;)
	{
...
//  mode->if_chassis_weak=1;
 ...
 {
  mode->chassis_mode=USART_Rx_data.chassis_mode;
//  mode->if_chassis_weak=1;

  mode->speed_usart_get.vx=limit_addspeed(speed_filter(USART_Rx_data.chassis_vx/WHEEL_FACTOR*2.5),mode->speed_usart_get.vx,100);//USART_Rx_data.chassis_vx;
  mode->speed_usart_get.vy= limit_addspeed(speed_filter(USART_Rx_data.chassis_vy/WHEEL_FACTOR*2.5),mode->speed_usart_get.vy,100);// USART_Rx_data.chassis_vy;
...
{
    if(mode->dm_ecd>(forword_ecd-3.141593)&&mode->dm_ecd<(3.141593))
    {
      mode->diff_angle=mode->dm_ecd-forword_ecd;
    }
    else if(mode->dm_ecd>(-3.141593)&&mode->dm_ecd<(forword_ecd-3.141593))
    {
      mode->diff_angle=3.141593-forword_ecd-mode->dm_ecd;
    }
   
   
   if(mode->dm_ecd>(forword_ecd-3.141593)&&mode->dm_ecd<forword_ecd)
   {
     mode->diff_angle=forword_ecd-mode->dm_ecd;
   }
    else if(mode->dm_ecd>forword_ecd&&mode->dm_ecd<(3.141593))
    {
      mode->diff_angle=2*3.1415926-(mode->dm_ecd-forword_ecd);
    }
    else if(mode->dm_ecd<(forword_ecd-3.141593)&&mode->dm_ecd>-3.141593)
    {
     mode->diff_angle=forword_ecd-mode->dm_ecd;
    }
    ...
    mode->diff_angle=2*3.1415926-mode->diff_angle;
    
    
    
    if(mode->dm_ecd<(forword_ecd-3.1415926)&&mode->dm_ecd>-3.1415926)
    {
      follow_ecd=forword_ecd-2*3.1415926-mode->dm_ecd;
    }
    else if(mode->dm_ecd>forword_ecd&&mode->dm_ecd<3.1415926)
    {
      follow_ecd=forword_ecd-mode->dm_ecd;
    }
    else if(mode->dm_ecd>(forword_ecd-3.1415926)&&mode->dm_ecd<forword_ecd)
    {
...
//     chassis_follow_set(mode);
  ...
  switch(mode->Vz_state)
  {
    ...
    case low_spine:
    {
      mode->speed_in.wz=1500*mode->wz_back;
     break;      
    }
    
        case mid_spine:
    {
      mode->speed_in.wz=3000*mode->wz_back;
     break;      
    }
        case high_spine:
    {
...
void printf_task(void const * argument)
{
  ...
  int i=0;
//  tttt++;
//printf("%f,%f,%f\n",(float)chassis_motor[3].motor_measure.speed_rpm,(float)chassis_motor[3].motor_tar.set_current,pm_power);
 printf("%f\n",(float)USART_Rx_data.if_arrived);
//printf("%f,%f,%f,%f\n",get_power,remain_power,USART_Rx_data.chassis_power,(float)USART_Rx_data.chassis_buff);
//printf("%f,%f,%f,%f\n",(float)steer_motor[i].motor_measure.ecd,chassis.steer_ecd[i],(float)steer_motor[i].motor_measure.speed_rpm,chassis.steer_speed_set[i]);
//printf("%f,%f,%f,%f\n",(float)USART_Rx_data.current_x,(float)USART_Rx_data.current_y,(float)(USART_Rx_data.chassis_vx/10),(float)(USART_Rx_data.chassis_vy/10));
//printf("%f,%f,%f,%f\n",(float)steer_motor[0].motor_measure.ecd,(float)lkld,(float)steer_motor[1].motor_measure.temperate,(float)chassis.chassis_mode);
//printf("%f,%f,%f\n",(float)USART_Rx_data.chassis_vx,(float)USART_Rx_data.chassis_vy,(float)location.Sy);
//printf("%f,%f,%f,%d\n",chassis.decay_ecd[i],chassis.steer_ecd[i],(float)steer_motor[i].motor_measure.ecd,ffff);
//printf("%d,%d,%d,%d,%f\n",(abs)(chassis_motor[0].motor_measure.speed_rpm),(abs)(chassis_motor[1].motor_measure.speed_rpm),(abs)(chassis_motor[2].motor_measure.speed_rpm),(abs)(chassis_motor[3].motor_measure.speed_rpm),real_chassis);
//printf("%f,%f,%f,%f\n",location.Sx,location.Sy,location.relative_angle,location.vx);
//printf("%f,%f,%f,%f\n",(float)USART_Rx_data.chassis_Power_limit,(float)USART_Rx_data.chassis_buff,SuperCAP.C_Vol,SuperCAP.Real_Power);
 osDelay(5);
